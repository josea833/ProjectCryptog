...
string aes_OFBCipher_HighLevel(string key, string message);
string des_OFBCipher_HighLevel(string key, string message);
string des3_OFBCipher_HighLevel(string key, string message);
...
int main(int argc, char *argv[]) {
    ...
    cout << "========================================" << endl;
    cout << "OFB MODE ENCRYPTION PERFORMANCE TESTING (HIGH-LEVEL API)" << endl;
    ...
    for (const auto &size : sizes) {
        ...
        des_OFBCipher_HighLevel("01234567", plaintext);
    }
    */
    ...
    for (const auto &size : sizes) {
        ...
        des3_OFBCipher_HighLevel("0123456789abcdef01234567", plaintext);
    }
    ...
    for (const auto &size : sizes) {
        ...
        aes_OFBCipher_HighLevel("0123456789abcdef", plaintext);
    }
    ...
    for (const auto &size : sizes) {
        ...
        aes_OFBCipher_HighLevel("0123456789abcdef01234567", plaintext);
    }
    ...
    for (const auto &size : sizes) {
        ...
        aes_OFBCipher_HighLevel("0123456789abcdef0123456789abcdef", plaintext);
    }
...
string aes_OFBCipher_HighLevel(string key, string message) {
    cout << "Using AES OFB Mode (High-Level EVP API)" << endl;
...
string des_OFBCipher_HighLevel(string key, string message) {
    cout << "Using DES OFB Mode (High-Level EVP API)" << endl;
    
    size_t key_len_bytes = key.length();
    if (key_len_bytes != 8) {
        cerr << "Error: Invalid key length. Must be 8 bytes, but got " << key_len_bytes << endl;
        return "";
    }
    ...
    // Manual OFB implementation using EVP_des_ecb
    ...
    while (offset < inputlength) {
        ...
        EVP_EncryptUpdate(ctx, keystream, &len, state, DES_BLOCK_SIZE);
    ...
    // Decrypt (same keystream generation path as encryption)
    EVP_CIPHER_CTX *ctx_dec = EVP_CIPHER_CTX_new();
    EVP_EncryptInit_ex(ctx_dec, EVP_des_ecb(), NULL, (unsigned char*)key.data(), NULL);
    ...
    while (offset < inputlength) {
        // Encrypt the state to get keystream block
        EVP_EncryptUpdate(ctx_dec, keystream, &len, state, DES_BLOCK_SIZE);
        // State becomes the keystream for the next iteration (OFB characteristic)
        memcpy(state, keystream, DES_BLOCK_SIZE);
...
string des3_OFBCipher_HighLevel(string key, string message) {
    cout << "Using 3DES OFB Mode (High-Level EVP API)" << endl;
    
    size_t key_len_bytes = key.length();
    if (key_len_bytes != 16 && key_len_bytes != 24) {
        cerr << "Error: Invalid key length. Must be 16 or 24 bytes, but got " << key_len_bytes << endl;
        return "";
    }
    ...
    // Manual OFB implementation using EVP_des_ede3_ecb or EVP_des_ede_ecb
    unsigned char state[DES_BLOCK_SIZE];
    memcpy(state, iv, DES_BLOCK_SIZE);
    unsigned char keystream[DES_BLOCK_SIZE];
    ...
    // Decrypt (same keystream generation path as encryption)
    EVP_CIPHER_CTX *ctx_dec = EVP_CIPHER_CTX_new();
    EVP_EncryptInit_ex(ctx_dec, cipher, NULL, (unsigned char*)key.data(), NULL);
